...
def process_proxy_data(proxy_data: ProxyData) -> None:
    connection = outbound_connections[proxy_data.stream_id]
    connection.sendall(proxy_data.payload)


def process_proxy_close(proxy_close: ProxyClose) -> None:
    if proxy_close.stream_id in outbound_connections:
        outbound_connection = outbound_connections[proxy_close.stream_id]
        del outbound_connections[proxy_close.stream_id]
...
def process_frame(connection: socket.socket, client_address: str, frame: Frame):
    if frame.frame_type == FrameType.PROXY_START:
        ...
        process_proxy_data(proxy_data=ProxyData.decode(frame.payload))
    elif frame.frame_type == FrameType.PROXY_CLOSE:
        process_proxy_close(proxy_close=ProxyClose.decode(frame.payload))
    else:
